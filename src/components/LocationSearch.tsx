...
function LocationSearch() {
  ...
  const handlePlaceClick = (place: WikiData) => {
    if (place.title === "User's Current Location") {
  ...
  return (
    ...
    <div className="p-4 bg-gray-100 min-h-screen">
          ...
          <div className="grid gap-4 mt-4">
            {results.map((place, index) => (
              <div key={index} className="bg-white shadow-md rounded-lg p-4 flex items-center">
                {place.thumbnail && (
                  <img src={place.thumbnail} alt={place.title} className="w-16 h-16 rounded mr-4" />
                )}
      ...
      <div className="col-span-9 ">
        {selectedPlace && <Map selectedPlace={selectedPlace} />}
      </div>
